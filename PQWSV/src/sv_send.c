...
static void SV_PrintToClient(client_t *cl, int level, char *string)
{
	...
	{
		Sys_Printf("SV_PrintToClient: client not ready. Client name is %s\n",cl->name);
		return;
	}
...
void SV_MulticastEx (vec3_t origin, int to, const char *cl_reliable_key)
{
	...
	for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++)
	{
		...
		// OfN Exclude non-prozac if needed, or the opposite
		if (to & MULTICAST_PROZAC)
		{
			if (client->CL_Kind != CLTYPE_PROZACQW)
				continue;
			if (client->CL_Version < minprozac)
				continue;
		}
		else if (to & MULTICAST_NON_PROZAC)
		{
			if (client->CL_Kind == CLTYPE_PROZACQW && client->CL_Version >= minprozac)
				continue;
		}
		// OfN End
		...
		if ((to & 7) == MULTICAST_PHS_R || (to & 7) == MULTICAST_PHS)
		{
			vec3_t delta;
...
inrange:
	...
	if (sv.mvdrecording && !(to & MULTICAST_PROZAC)) // Added prozac multicast exclusion condition
	{
		if (reliable)
		{
...
void SV_UpdateClientStats (client_t *client)
{
	...
	else // Mod that doesnt use a "runes_owned" field.. (standard behaviour) - OfN end
