...
cvar_t pm_deadfix = { "pm_deadfix","0" }; // December 2021
...
qbool SV_RunThink (edict_t *ent)
{
	...
	do
	{
		...
		// May 2021 - OfN Spoike's fix for infinite loops, freezing the server on 'self.nextthink = time;' quakeC code items
		if (ent->v.nextthink <= thinktime && ent->v.nextthink > 0)
		{
			//Con_Printf("Infinite loop for \"%s\" entity\n", E_STRING(ent, ent->v.classname));
			Con_DPrintf("Infinite loop for \"%s\" entity\n", PR1_GetString(ent->v.classname));
			return true;
		}
...
void SV_CheckWaterTransition (edict_t *ent)
{
	...
	{
		//if (ent->v.watertype == CONTENTS_EMPTY) // Original code before December 2021
...
void SV_Physics_Toss (edict_t *ent)
{
	...
	// May 2021
	if ((int)ent->v.flags & FL_FLYNOCLIP) // Go through walls if entity is flagged to do so..
		return;
	// End May 2021
	...
	// December 2021 TOCHECK: Fixes rotating heads/gibs sometimes with no regular movement?
...
void SV_Physics_Step (edict_t *ent)
{
	...
	if ( ! ((int)ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM) ) )
	{
		...
		{
			if (hitsound && !((int)ent->v.flags & FL_SKIPLANDSAMPLE)) // May 2021 - OfN Added the new flag to skip land sample for army soldier bots jumping in prozac
