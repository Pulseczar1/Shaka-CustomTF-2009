...
dstatement7_t   *pr_statements7; // OfN - Migration
globalvars_t	*pr_global_struct;
float			*pr_globals;			// same as pr_global_struct
int				pr_edict_size;	// in bytes

// OfN - Migration
qbool prozacmod_loaded;
// OfN end
...
void PR1_LoadProgs (void)
{
	...
	// OfN - Migration
	if (!progs)
		progs = (dprograms_t *)FS_LoadHunkFile("qwprozac.dat", &filesize);
	// OfN End
	if (!progs)
	...
	if (progs->version != PROG_VERSION && progs->version != PROG_VERSION2)
		SV_Error ("qwprogs.dat has wrong version number (%i should be %i or %i)", progs->version, PROG_VERSION, PROG_VERSION2); // OfN added version2
	if (progs->crc != (pr_nqprogs ? NQ_PROGHEADER_CRC : PROGHEADER_CRC))
		//SV_Error ("You must have the qwprogs.dat from QuakeWorld installed");
		SV_Error("Progs CRC value differs, not compatible"); // OfN - Original above
	...
	// OfN - Migration - Commented out, and moved up ->> 
	// pr_statements = (dstatement_t *)((byte *)progs + progs->ofs_statements);
	...
	// byte swap the lumps
	if (pr_statements) // OfN
	{
		for (i = 0; i < progs->numstatements; i++) // OfN original code here
		{
	...
	if ((f = ED_FindFunction("ClientCommand")) != NULL) //OfN
		...
		ClientCommand = (func_t)NULL;

	if ((f = ED_FindFunction("Teammate")) != NULL) //OfN
		Teammate_pf = (func_t)(f - pr_functions);
	else
		Teammate_pf = (func_t)NULL;
