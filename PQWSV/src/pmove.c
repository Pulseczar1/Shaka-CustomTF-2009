...
static void PM_Friction(void)
{
	...
	if (speed < 1) {
		...
		return;
	                        ...
	                        //from here December 2021
	if (pmove.waterlevel >= 2 && !(pmove.pm_type == PM_DEAD && pm_deadfix.value)) {
...
static int PM_AirMove(void); // December 2021, the declaration here
static int PM_FlyMove(void); // December 2021, the declaration
...
static int PM_WaterMove(void)
{
	...
	//=======================================================//
	// December 2021 Ugly hack for moving player dead heads on water
	if (pmove.pm_type == PM_DEAD && pm_deadfix.value)
		return PM_AirMove(); // NO->PM_StepSlideMove(true);// PM_AirMove();//no->PM_FlyMove();//PM_AirMove(); // TOCHECK: Would be better PM_StepSlideMove(false); ? or PM_FlyMove() ?
	//======================================================//
...
static void PM_CheckJump (void)
{
	...
	if (pmove.waterlevel >= 2) {
		...
		// December 2021
		if ((int)sv_player->v.flags & FL_UWATERPUSHED)
			return;
		// End december 2021
