...
static void SV_CreateBaseline (void)
{
	...
	for (entnum = 0; entnum < sv.num_edicts ; entnum++)
	{
		...
		{
			svent->e->baseline.colormap = 0; //svent->v.colormap; // Was  =0 before July 2021
...
void SV_SpawnServer(char *mapname, qbool devmap, char* entityfile, qbool loading_savegame)
{
	...
	if ((int)sv_loadentfiles.value)
	{
		...
		// OfN - VERY FIRST check on "ents" folder
		snprintf(ent_path, sizeof(ent_path), "ents/%s.ent", entityfile);
		entitystring = (char *)FS_LoadHunkFile(ent_path, NULL);

		// first try maps/sv_loadentfiles_dir/
		if (!entitystring && sv_loadentfiles_dir.string[0])
		{
		...
		if (entitystring) {
			...
			// ADDED OfN - Check if strlen() on this is safe or not, it seems so, the FS loading function 0-terminates the data
			Info_SetValueForStarKey(svs.info, "*entfile", va("%i", CRC_Block((byte *)entitystring, strlen(entitystring))), MAX_SERVERINFO_STRING);
		}
	}

	if (!entitystring) {
		Info_SetValueForStarKey(svs.info, "*entfile", "", MAX_SERVERINFO_STRING);// ADDED OfN
