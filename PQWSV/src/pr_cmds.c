...
void PF_sprint (void)
{
	...
	{
		// ORIGINAL CODE before January 2021 -> Con_Printf ("tried to sprint to a non-client\n");

		// Added January 2021
		PR_RunError("tried to sprint to a non-client. Text to be printed was: \"%s\"\n",s);
		// END - Added January 2021
...
single centerprint to a specific client
...
// touch linked edicts
	...
	// The following is from old cpqwsv
	edict_t	*ent, *chain;
	float	rad;
	float	*org;
	vec3_t	eorg;
...
//==============================================================================
// OfN Old School builtins from migration from cpqwsv
//==============================================================================
...
Progs_StrBufferItem ProgsStringsBuffer[MAX_BUFFER_STRINGS]; // Added January 2022
unsigned int NumStrBuffersUsed;// Added January 2022
Progs_PermStringItem ProgsPermString[MAX_PERM_STRINGS];// Added January 2022
...
char* UsePermString(char* thestring)
{
	...
	if (strlen(thestring) > MAX_PERM_STRING_LEN - 2)
		had_error = true;
	...
	for (i = 0; i < MAX_PERM_STRINGS; i++)
	{
		if (!ProgsPermString[i].isUsed)
		{
			...
			if (had_error)
				strcpy(thestring, "String too long for perm buffer!");
			else
				strcpy(ProgsPermString[i].str, thestring);
...
void PF_strcatOfN(void)
{
	...
	if (strlen(st1) + strlen(st2) > MAX_BUFFER_STRINGS_LEN - 2)
		strcpy(result, "String too long for strcat()");
	else
...
void PF_strcasecmp(void)
{
	...
	retval = (float)strcasecmp(st1, st2);
...
void PF_getclient(void)
{
	...
	if (!ent)
	{
		...
		if (uid != 0) // was it even a number?
		{
			...
			{
				if (cl->edict->e->free || cl->state < cs_spawned) // Added July 2020
					continue;
...
void PF_validatefile(void)
{
	...
	fileinfo = FS_OpenVFS(st, "rb", FS_GAME);
...
void(entity e, float chan, string samp, float vol, float atten) clientsound
==============
*/
...
void(float prozacver) setminprozac
==============
*/
// Sets the minimum protocol prozac client global
void PF_setminprozac(void)
{
	minprozac = G_FLOAT(OFS_PARM0);
}
...
==============
*/
// Gets the total seconds the server has been up
void PF_getrealtime(void)
{
	float retval;

	retval = (float)curtime;
...
void PF_linkedict(void)
{
	float option;
	edict_t* ent;

	ent = G_EDICT(OFS_PARM0);
	option = G_FLOAT(OFS_PARM1);
...
{145, PF_setminprozac},
{146, PF_getrealtime},
{147, PF_linkedict}, // December 2021
// End OfN
