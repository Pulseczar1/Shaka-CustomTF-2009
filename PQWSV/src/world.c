...
// touch linked edicts
	while (1)
	{
		...
		for (l = start->next ; l != start ; l = l->next)
		{
			...
			if (touch->v.solid == SOLID_NOT) // OfN Added the && blah
				continue;
	...
	if (clip->type == TL_EVERYTHING)
	{
		...
		for (i = 1; i<sv.num_edicts; i++, touch = NEXT_EDICT(touch))
		{
			...
			if ((int)touch->v.flags & FL_MONSTER)
				trace = SV_ClipMoveToEntity(touch,&touch->v.origin, clip->start, clip->mins2, clip->maxs2, clip->end);
			else
				trace = SV_ClipMoveToEntity(touch, &touch->v.origin,clip->start, clip->mins, clip->maxs, clip->end);
