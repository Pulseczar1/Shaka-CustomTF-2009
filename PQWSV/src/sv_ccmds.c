...
char CurrentIntegrationStr[512]; // Added on January 2022, moved from server.h which was not extern
...
void SV_Map (qbool now)
{
	...
	if (now)
	{
		...
		// Display Edicts and precaches usage/limits
		SV_PrecacheUsage_f();
		SV_EntUsage_f();

		// Retrieve map world integration entity if any
		CurrentIntegrationStr[0] = 0; // OfN - Migration
		if (SV_RetrieveMapIntegration()) //<-- this caused linux to crash, not anymore!
			Con_Printf("Found World Integration entity\n");
		else
			Con_Printf("World Integration entity not found\n");
		// OfN - End
		...
		SV_BroadcastIntegrationToCompatibleClients(); // OfN
#else
...
void SV_SavePins(qbool silent)
{
	...
	//if (strlen(Cvar_VariableString("sv_pinsfile")) == 0)
	if (strlen(sv_pinsfile.string) == 0)
	{
		if (!silent)
	...
	sprintf(name, "%s/%s", fs_gamedir, sv_pinsfile.string);
...
Displays the time up for the server
==================
*/

void SV_ShowUpTime_f(void)
{
	char* tmonth, * suffix;
	char tday[12];
	int edays, ehours, emins;
	
	switch (startup_time.month)
	{
	case 0:
		tmonth = "January";
		break;
	...
	sprintf(tday, "%d%s", startup_time.day, suffix);
		
	Con_Printf("Server started on: %s %s of %d\n", tmonth, tday, startup_time.year);

	Con_Printf("Time at instance start: %02d:%02d:%02d\n", startup_time.hour, startup_time.minute, startup_time.second);

	emins = (int)(curtime / 60);
	ehours = (int)(emins / 60);
	edays = (int)(ehours / 24);
	...
	Con_Printf("Period elapsed running: %d days, %d hours and %d minutes.\n",edays,ehours,emins);
}
...
void SV_ShowVersion_f(void)
{
	Con_Printf("Prozac server version: %s (MVDSV %s) (QW %s) %s OS\n", VERSION_PQWSV, VERSION_NUMBER, QW_VERSION, QW_PLATFORM);
	Con_Printf("Executable compiled on: "__TIME__" - "__DATE__"\n");
}
...
void SV_Pinmsg_f(void)
{
	...
	if (name == NULL) // If no name assume it's from server console
		name = "Console";
	...
	if (strcasecmp(Cmd_Argv(1), "view") == 0)
	{
	...
	else if (strcasecmp(Cmd_Argv(1), "clear") == 0)
	{
		if (strcasecmp(Cmd_Argv(2), "all") == 0)
		{
			isbegin = true;
	...
	else if (strcasecmp(Cmd_Argv(1), "save") == 0)
	{
		SV_SavePins(false); // not silent
	}
	else if (strcasecmp(Cmd_Argv(1), "load") == 0)
	{
		SV_LoadPins();
	}
	else if (strcasecmp(Cmd_Argv(1), "stats") == 0)
	{
		isbegin = true;
		for (i = 0; i < NUM_PINMSGS; i++)
		{
	...
	else if (i >= 0 && i < NUM_PINMSGS && Cmd_Argv(2) != "")
	{
		...
		{
			Con_Printf("Not allowed without being connected!\n");
			return;
		}

		if (Cmd_Argc() >= 4)
		{
			if (strlen(pinmsg_cmdbuffer) >= MAX_PINMSGITEM_STRING - 2)
			{
		...
		{
			if (strlen(Cmd_Argv(2)) >= MAX_PINMSGITEM_STRING - 2)
			{
				Con_Printf("Text message too long!\n");
				return;
			}
		...
		if (Cmd_Argc() >= 4)
			strcpy(pinmsg[i].msg, pinmsg_cmdbuffer);
		else
			strcpy(pinmsg[i].msg, Cmd_Argv(2));

		pinmsg[i].counter = 0;

		if (strlen(name) + strlen(timestamp) + 11 >= MAX_PINMSGSIGN_STRING - 2)
		{
			strcpy(pinmsg[i].sign, "(Error on signature of message)");
			Con_Printf("Error signing message!\n");
		}
		else
		{
			strcpy(pinmsg[i].sign, "Set on ");
			strcat(pinmsg[i].sign, timestamp);
			strcat(pinmsg[i].sign, " by ");
			strcat(pinmsg[i].sign, name);
		}
	}
	else
...
// I wonder why it registered in host.c in ezquake...
	...
	Cmd_AddCommand("pinmsg", SV_Pinmsg_f); // OfN
	Cmd_AddCommand("ents", SV_EntUsage_f);
	Cmd_AddCommand("precaches", SV_PrecacheUsage_f);
	Cmd_AddCommand("showversion", SV_ShowVersion_f);
	Cmd_AddCommand("uptime", SV_ShowUpTime_f);
}
